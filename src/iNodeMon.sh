...
_start_auth()
{
    ...
    if [[ ! -z "${IfExist}" ]]; then
        ...
        sudo /Applications/iNodeClient/${AUTHMNGSERVICE} >& /dev/null &
    fi
    sleep 1
...
_kill(){
    ...
    KCMD="sudo ${PKILLCMD} ${process}"
    eval "${KCMD}"
    _get_pid "${process}"
    if [[ ! -z "${pid}" ]]; then
        if [[ "${_old_pid}" == "${pid}" ]]; then
            _log -e "Failed to kill ${process}, you can mannually kill by using \"sudo ${KCMD}\""
        else
            _log -e "Failed to kill ${process}, it restarted automatically."
        fi
    else
